 ...
 *       required: true
 *     },
 *     fullName2:"(Voornaam + ' ' + Tussenvoegsel) + ' ' + Achternaam",
 *     fullName3: "(col(3) + ' ' + col(2)) + ' ' + col(1)",
 *     telephone: {
 *       _index: "'0'",
 ...
 *
 *
 * if we use jexl.eval we must translate the fieldnames into the index
 *
 * so jexl.eval('Achternaam') should read the column Achternaam but we only know an array
 * so we should make an object with the row => value version of that row
 *   so obj = {IdentificatieMedewerker: 1023040, SamengesteldeNaam: 'Jan de Hond'}
 * but that will be a lot of movement.
 *
 * if we write jexl.eval('column("Achternaam")' then it's easy
 * So with the objec
 *
 * should create from
